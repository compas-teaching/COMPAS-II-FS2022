...
from compas_fab.robots import AttachedCollisionMesh, CollisionMesh, PlanningScene, Tool
...
def get_pick_frame(part_key):
    return Frame((0.5, 0.01, 0.02), (-1, 0, 0), (0, 1, 0))
...
def prepare_scene(robot, part_key):
    ...
    box = Box.from_diagonal([(-0.5, -0.5, 0), (0.5, 0.5, -0.001)])
...
def get_start_configuration(robot):
    ...
    config.joint_values = (0, 5, 1.57, -1.57, -1.57, 1.57)
    return config
...
for part_key in get_part_sequence()[0:7]:   # NOTE: For quick testing, only take the first few parts in the sequence
    part = assembly.graph.node_attribute(part_key, "part")
