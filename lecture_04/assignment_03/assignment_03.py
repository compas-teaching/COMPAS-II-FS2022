...
def calculate_ik_for_frames(robot, frames):
    ...
    T = Translation.from_vector([0.1,0,0.1])
    frame_0 = frames[0].transformed(T)

    start_configuration = robot.zero_configuration()

    # Configuration.joint_values = (0,-math.pi/2,0,-math.pi/2,0,0)
    # configs.append(start_configuration)

    configuration_1st = robot.inverse_kinematics(frame_0, start_configuration)
    configs.append(configuration_1st)


    for i,f in enumerate(frames):
        c = robot.inverse_kinematics(f,configs[i])
        configs.append(c)
